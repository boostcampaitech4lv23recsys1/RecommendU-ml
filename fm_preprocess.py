...
def map_answerlog_last_reclog(answerlogs, reclogs):
    answerlogs["label"] = 1
...
def make_negative_answerlog(fe_concat):
    fe_data = fe_concat[['rec_timestamp', 'answer_id', 'rectype_id',
...
def merge_side_information(conn, fe_data):
    #### user data merge ####
    ...
    fe_data = fe_data.merge(use_user_data,how="left", on="user")
    ...
    fe_data = fe_data.merge(answer_with_doc,how="left", on="answer")

    fe_data.rename(columns={"doc_major_small_id":"doc_major_small", "doc_major_large_id":"doc_major_large", 
    ...
    fe_data = fe_data.merge(rec_jobsmall, how="left", on="rec_job_small")
    fe_data = fe_data.merge(doc_jobsmall, how="left", on="doc_job_small")

    #### 정렬 및 rename ###
    fe_data = fe_data.sort_values(by=['rec_log_id'])
    fe_data = fe_data[["user","user_career_type","user_job_large","user_major_small","rectype","rec_company","rec_job_large","rec_job_small","rec_question_type",
    ...
    #### coin 추가 ####
    fe_data.loc[fe_data["rec_company"]==fe_data["doc_company"], "coin_company"] = 1
    fe_data.loc[fe_data["rec_company"]!=fe_data["doc_company"], "coin_company"] = 0

    fe_data.loc[fe_data["rec_job_large"]==fe_data["doc_job_large"], "coin_joblarge"] = 1
    fe_data.loc[fe_data["rec_job_large"]!=fe_data["doc_job_large"], "coin_joblarge"] = 0

    fe_data.loc[fe_data["rec_job_small"]==fe_data["doc_job_small"], "coin_jobsmall"] = 1
    fe_data.loc[fe_data["rec_job_small"]!=fe_data["doc_job_small"], "coin_jobsmall"] = 0
    ...
    for i in range(len(fe_data)):
        answer = answer_question_list[fe_data.loc[i, 'answer']]
        if fe_data.loc[i, 'rec_question_type'] in answer:
            fe_data.loc[i, 'coin_question_type'] = 1
        else:
            fe_data.loc[i, 'coin_question_type'] = 0
